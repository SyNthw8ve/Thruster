...
fuel_train = Fuel2D(folder=data_folder, file=file_name, num_instances=num_instances)
fuel_eval = Fuel2D(folder=data_folder, file=file_name, num_instances=num_instances)

injector_train = Injector2D(fuel=fuel_train, min_quantity=10, max_quantity=num_instances)
injector_eval = Injector2D(fuel=fuel_eval, min_quantity=10, max_quantity=num_instances)
...
trainer.run(replay_buffer_max_length=100000, num_iterations=20000,
            ...
            initial_collect_steps=1000, policy_save_path='./policies/q_rnn_dynamic')
